...
# GO FUNCTION: Enters the folder containing the letters you entered, ignores case. 
# E.g: go piec > enters the folder One Piece.
...
alias cs='clear && ls --color=auto -ASsh && exec bash' # cs = list files by size; only files, ignores folders. 
alias size='clear && df -h .; echo "."; sudo du -h --max-depth=1 | sort -hr' # List files by size, counting folders.
...
alias nf='echo "" && echo "Files: $(find . -maxdepth 1 -type f | wc -l)" && echo "Folders: $(find . -maxdepth 1 -type d | wc -l)" && echo "Total: $(find . -maxdepth 1 | wc -l)" && echo ""' #nf = Number of files
